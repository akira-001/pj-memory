...
[English](https://github.com/akira-001/pj-memory/blob/main/README.md)
...
- **行動強制フック**: Claude Code hooksによるリアルタイムガードレール — `skill-gate` がファイルパターン→スキルの対応をチェック、`failure-breaker` が連続コマンド失敗を検知
...
- **Suggest → Promote**: 繰り返し現れるパターンをスキル作成候補として記録し、準備ができたらプロモート
...
cogmem skills suggest "deploy-pattern" --description "繰り返しのデプロイ手順"
cogmem skills suggest-summary                 # 繰り返し候補を表示
cogmem skills promote "deploy-pattern"        # プロモート（スキル作成済み）
```

### 9. 行動強制フック（v0.19.0）

Claude Code hooks によるエージェント行動のリアルタイムガードレール:

- **skill-gate**（PreToolUse）: エージェントが `[[cogmem.skill_triggers]]` パターンに一致するファイルを編集する際、対応スキルが未読み込みなら警告
- **failure-breaker**（PostToolUse）: Bash の連続失敗を検知し、立ち止まって考え直すよう促す

フックは `cogmem.toml` の設定で制御 — ハードコードされたチェックは一切なし:
...
フックは `cogmem init` と `cogmem migrate` で自動登録される。
...
1. **エージェントスキル**（v0.14.0+）: 7つのプロトコルスキルを `~/.claude/skills/` にインストール — `session-init`、`live-logging`、`skill-tracking`、`wrap`、`crystallize`、加えて `search` と `recall`（v0.24.0 で追加）。これらはエージェントの行動プロトコルと横断記憶検索を動かし、すべてのプロジェクトで共通して機能します。再 init では既存スキルは保護されます。テンプレートの更新を取り込むには `cogmem migrate`（または `cogmem skills update-templates`）を使います。
...
├── cogmem.toml                  # 設定ファイル
├── .claude/
│   └── settings.json            # Claude Code hooks（自動登録）
├── identity/
...
cogmem upgrade-check                         # PyPI に新しい cogmem-agent があるか確認（24hキャッシュ・fail-open）
...
cogmem skills review                         # 全スキルのヘルスレポート
cogmem skills suggest "パターン名"           # スキル作成候補を記録
cogmem skills suggest-summary                # 繰り返し候補を表示
cogmem skills promote "パターン名"           # 候補をプロモート
cogmem skills update-templates               # ~/.claude/skills/ をパッケージ同梱テンプレートと同期
...
任意のバージョン（旧版でも最近のでも）から:

```bash
pip install --upgrade cogmem-agent
cogmem migrate                    # --lang は cogmem.toml から自動検出（v0.29.0+）
```

`cogmem migrate` 1コマンドで既存プロジェクトを最新状態にする。実行内容:

- **アイデンティティ移行**（旧版プロジェクト）: `identity/agent.md` → `identity/soul.md` にリネーム、`identity/agents.md` を新規生成、`cogmem.toml` と `CLAUDE.md` の参照を更新
- **`user_id` 移行**: `cogmem.local.toml` に `user_id` を書き込み、ログ・コンテキストをユーザー別サブディレクトリへ移動
- **フック登録**: `.claude/settings.json` に Claude Code フックを書き込み
- **言語永続化**: `[cogmem].lang` を `cogmem.toml` に保存（v0.28.0+）
- **skill テンプレート同期**（v0.27.0+）: 既存 `~/.claude/skills/<name>/SKILL.md` と同梱テンプレートの diff を検出 → `[y/N/d=diff表示]` で個別確認 → `~/.claude/skills/.backup-YYYY-MM-DD/` へ自動バックアップ。`--auto-yes-skills` で全自動、`--no-skills` でスキップ可能。

同等のショートカット:

```bash
cogmem init                       # 既存プロジェクトで「migrate を実行しますか？ [Y/n]」プロンプト → y で migrate に流れる
cogmem skills update-templates    # skill 同期ステップだけを実行（--dry-run でプレビュー）
```

#### skill テンプレートのライフサイクル

プロジェクト初期化後、リリースを跨いで同梱 skill テンプレートが進化する場合があります（trigger 追加、Step 追加、修正など）。`cogmem init` は **skip 保護**で既存スキルを上書きしません。`pip install -U cogmem-agent` 後に template の改善を取り込むには、`cogmem migrate`（または `cogmem skills update-templates`）を実行します。drift 検出は `[cogmem].lang` を尊重するため、`--lang ja` で初期化したプロジェクトでは `templates/ja/skills/` だけを比較対象とします。

#### アップグレード通知

`cogmem upgrade-check` は PyPI を見て新しい cogmem-agent があるかチェックします（`cogmem.toml [updates].last_check` で 24h キャッシュ、ネットワークエラー時は fail-open）。`session-init` skill が毎セッション呼び出して、以下のように通知します:
...
```bash
cogmem upgrade-check --snooze-days 30   # 通知を延期
# または cogmem.toml に:
[updates]
auto = "never"                          # 永久に通知しない
```
