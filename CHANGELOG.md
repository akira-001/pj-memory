...
- **identity_user_path fallback**: When `user_id` is set but per-user file (`identity/users/{user_id}.md`) does not exist, fall back to shared `identity/user.md` instead of returning a non-existent path
