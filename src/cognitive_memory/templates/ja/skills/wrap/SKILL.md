...
## Step 3.8: 行動パターンレビュー（未スキル化ワークフローの検知）

a. `auto_improve = "off"` → スキップ
b. `cogmem watch --since "8 hours ago" --json` の `workflow_patterns` を確認（threshold=2）
c. エージェント自身の内省も併用（同じ手順を2回以上繰り返したか、既存スキルにないワークフローを実行したか）
d. 該当しない場合はスキップ
e. 該当する場合:
   - `"ask"`: 「[パターン名] をスキル化する？」とユーザーに確認。承認されたら作成
